 ...
 *
 * Noise comes from the shared liquid.glsl chunk (prepended at build time),
 * so the background rides the same wave language as the hero and cards.
 */
...
void main() {
  ...
  float f1 = lqFbm(p * 1.3 + vec2(t * 0.11, t * 0.07));
  float f2 = lqFbm(p * 1.8 - vec2(t * 0.05, t * 0.13) + f1 * warp * 1.5);
  float f3 = lqFbm(p * 0.9 + vec2(-t * 0.08, t * 0.06) + f2 * warp * 1.1);
  ...
  /* subtle film grain */
  float g = lqHash(gl_FragCoord.xy + fract(t) * 337.0);
  col += (g - 0.5) * uGrain;
